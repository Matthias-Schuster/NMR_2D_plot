...
def save_and_clear(fig, folder, name, p):
    ...
    # Save PNG and SVG
    if p["save_png"]:
        fig.savefig(save_base.with_suffix(".png"), transparent=False,
                    bbox_inches="tight", dpi=p.get('dpi', 300))
    if p["save_svg"]:
        fig.savefig(save_base.with_suffix(".svg"), transparent=False,
                    bbox_inches="tight")
    plt.show()
    plt.close(fig)
...
def apply_formatting(ax, p, title=None, is_grid=False, add_labels=False):
    ...
    if add_labels and p['labels']:
        ...
        ax.set_ylabel(p['y_label'], loc="top", labelpad=p['labelpad_y'],
                      rotation="horizontal", ma="center", backgroundcolor="w")
...
def draw_contours(ax, dic, data, p, contour_start, color):
    ...
    if cache_key in _CONTOUR_CACHE:
        ...
        lc_pos = LineCollection(cached['pos_paths'], colors=color,
                                linewidths=p['line_width'], alpha=p['alpha'])
        ax.add_collection(lc_pos)

        # Draw Negative Contours
        if p['negative'] and cached['neg_paths']:
            lc_neg = LineCollection(cached['neg_paths'], colors=p['neg_color'],
                                    linewidths=p['line_width'], alpha=p['alpha'],
                                    linestyles='dashed')
    ...
    if negative:
        ...
        cln = ax.contour(data, neg_levels,
                         colors=p['neg_color'],
                         linewidths=p['line_width'],
                         extent=extent,
                         alpha=p['alpha'])
...
def grid_plot(p, row=2, col=2, folder="results"):
    ...
    fig, axes = plt.subplots(row, col, dpi=p['dpi'], figsize=(p['xsize']*col, p['ysize']*row),
...
def grid_plot_over(p, over, row=2, col=2,
                   folder="results", reverse=False):
    dic_all = p['dic_all']
    data_all = p['data_all']
    result_folder(folder)
    ...
    for i, ax in enumerate(axes_flat):
        if i < len(indices):
            ...
            if not reverse:
                ...
                draw_contours(ax, dic_all[over], data_all[over], p,
                              p['cont'][over], p['colors'][over])
            if reverse:
    ...
    for i, ax in enumerate(axes_flat):
        if i < len(overlay_groups):
            ...
            for spec_idx in overlay_groups[i]:
                ...
                clp = draw_contours(ax, dic_all[spec_idx], data_all[spec_idx],
                                    p, p['cont'][spec_idx], p['colors'][spec_idx])
                h, _ = clp.legend_elements()
                if h:
                    h_all.append(h[0])

            if h_all:
                leg = ax.legend(h_all, subplot_names, loc="upper left",
...
def read_hsqc_bruker_pdata(dic, data):
    x_ppm = _ppm_axis(data.shape[1], dic["procs"]["OFFSET"],
                      dic["procs"]["SW_p"], dic["procs"]["SF"])
    y_ppm = _ppm_axis(data.shape[0], dic["proc2s"]["OFFSET"],
                      dic["proc2s"]["SW_p"], dic["proc2s"]["SF"])
    return x_ppm, y_ppm
